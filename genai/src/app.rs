...
impl App {
    ...
    pub fn start_artnet_client(&mut self, cx:&mut Cx){
        ...
        if let Ok(result) = std::fs::read_to_string("./local/dmx/dmx.ron"){
                ...
                */
                ...
                if new_buttons.write_preset{ // write a preset
                    if let Some(idx) = new_buttons.preset_id(){
                        ...
                        std::fs::write(format!("./local/dmx/dmx{}.ron", idx), state.serialize_ron().as_bytes()).unwrap();
                    }
                }
                else{ // read a preset
                    if let Some(idx) = new_buttons.preset_id(){
                        if let Ok(result) = std::fs::read_to_string(format!("./local/dmx/dmx{}.ron", idx)){
                            if let Ok(load) = State::deserialize_ron(&result){
                                let ts = state;
                                state = load;
                                for i in 0..8{
